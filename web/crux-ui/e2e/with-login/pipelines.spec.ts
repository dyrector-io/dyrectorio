...
test.describe('Pipeline', () => {
  test('Can create azure pipeline', async ({ page }) => {
    const name = 'test'
    
    await page.goto(TEAM_ROUTES.pipeline.list())
    await page.waitForSelector('h2:text-is("Pipelines")')
  
    await page.locator('button:has-text("Add")').click()
    await expect(page.locator('h4:has-text("New pipeline")')).toHaveCount(1)
