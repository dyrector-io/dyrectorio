...
export default class ContainerMapper {
  ...
  dbConfigToCreateConfigStatement(
    ...
    return {
      ...
      proxyBuffering: toNullableBoolean(config.proxyBuffering),
      proxyHeaders: toPrismaJsonArray(config.proxyHeaders),
      useLoadBalancer: toNullableBoolean(config.useLoadBalancer),
      corsHeaders: toPrismaJsonArray(config.corsHeaders),
      ...
      replicas: toNullableNumber(config.replicas),
      experimental: toPrismaJson(config.experimental),
    }
  }
  ...
  configDataToDbPatch(config: ContainerConfigData): ContainerConfigDbPatch {
    return {
      ...
      proxyBuffering: 'proxyBuffering' in config ? toNullableBoolean(config.proxyBuffering) : undefined,
      proxyHeaders: 'proxyHeaders' in config ? toPrismaJsonArray(config.proxyHeaders) : undefined,
      useLoadBalancer: 'useLoadBalancer' in config ? toNullableBoolean(config.useLoadBalancer) : undefined,
      corsHeaders: 'corsHeaders' in config ? toPrismaJsonArray(config.corsHeaders) : undefined,
      ...
      experimental: 'experimental' in config ? toPrismaJson(config.experimental) : undefined,
    }
  }
