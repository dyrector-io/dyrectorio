...
const stringsConflict = (one: string, other: string): boolean => one && other && one !== other
...
const storagesConflict = (one: StorageData, other: StorageData): boolean => {
  if (!one || !other) {
    return false
  }
  ...
  return stringsConflict(one.storageId, other.storageId) || objectsConflict(one.storageConfig, other.storageConfig)
}
...
export const checkForConflicts = (
  ...
  interestedProperties: ContainerConfigDataProperty[] = [],
  ...
  configs = configs.map(conf => {
    ...
    Object.keys(conf).forEach(it => {
      ...
      if (prop === 'secrets') {
        // secrets can not be overriden
        return
      }

      if (interestedProperties.includes(prop)) {
        return
      }
...
export const getConflictsForConcreteConfig = (
  ...
  checkForConflicts(configs, Object.keys(concreteConfig) as ContainerConfigDataProperty[])
