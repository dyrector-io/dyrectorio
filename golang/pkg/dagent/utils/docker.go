...
func saveSecretsToVault(ctx context.Context, dog *dogger.DeploymentLogger,
	...
	_, created, err := vault.UpsertSecureNote(session,
		...
		fmt.Sprintf("generated by dagent @ %s", version.Version),
		vaultCfg.OrgID,
		[]string{vaultCfg.CollectionID},
		secrets,
	)
	if err != nil {
		dog.WriteInfo(fmt.Sprintf("Notice: vault secret backup failed: %s", err.Error()))
		return nil
	}

	if created {
		dog.WriteInfo(fmt.Sprintf("Vault secret created: %s/%s", prefix, name))
	} else {
		dog.WriteInfo(fmt.Sprintf("Vault secret updated: %s/%s", prefix, name))
	}
