...
func (s *Secret) BackupSecretsToVault(namespace, containerName string, secrets map[string]string) error {
	vaultCfg := s.appConfig.SecretVault

	// Decrypt secrets
	decrypted, err := crypt.DecryptSecrets(secrets, &s.appConfig.CommonConfiguration)
	if err != nil {
		return fmt.Errorf("failed to decrypt secrets for vault backup: %w", err)
	}
	...
	err = vault.LoginAPIKey(s.ctx, vaultCfg.URL, vaultCfg.ClientID, vaultCfg.ClientSecret)
	if err != nil {
		return err
	}

	session, err := vault.Unlock(s.ctx, vaultCfg.Password)
	if err != nil {
		return err
	}

	if err := vault.Sync(s.ctx, session); err != nil {
		return err
	}

	// Backup to vault
	_, _, err = vault.UpsertSecureNote(s.ctx,
		session,
