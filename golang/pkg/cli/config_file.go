	...
	if err != nil {
		...
		case errors.Is(err, containerRuntime.ErrServerIsOutdated):
			NotifyOnce("dockerversion", func() {
				log.Warn().Stack().Err(err).Msg("There is a newer version of the container engine in use, please consider updating.")
			})
		case errors.Is(err, containerRuntime.ErrServerVersionIsNotSupported):
			log.Fatal().Stack().Err(err).Msg("There is a newer version of the container engine in use, please consider updating.")
		default:
			log.Fatal().Stack().Err(err).Send()
		}
	}
