...
func PreflightChecks(config *config.Configuration) {
	...
	constraints, _ := version.NewConstraint(fmt.Sprintf(">=%s", config.MinDockerServerVersion))
	if err != nil {
		log.Println("Error with version constraint ", err.Error())
	}
	if !constraints.Check(serVer) {
		log.Printf("WARNING: server is behind the supported version %s < %s", serVer, config.MinDockerServerVersion)
	}
}
